...
<div class="ml-[210px] max-w-[1460px]">
    ...
    <section class="mt-5 rounded-xl border border-[#dedfe4] bg-white px-8 py-7 shadow-[0_2px_5px_rgba(0,0,0,0.18)]">
        <div class="grid gap-6 lg:grid-cols-[1fr_330px]">
            <div>
                ...
                <div class="mt-5 flex flex-wrap items-center gap-3 text-[18px]">
                    <span class="font-medium text-black">Materi:</span>
                    @forelse($pertemuan->materi as $materi)
                        <a href="{{ $materi->url }}" target="_blank" class="inline-flex min-h-[46px] items-center gap-3 rounded-lg border border-[#8d8d8d] bg-[#fbfbfc] px-4 py-2 text-[15px] font-medium text-[#222] transition hover:bg-gray-50">
    ...
    <section class="mt-7 overflow-hidden rounded-xl border border-[#dedfe4] bg-white shadow-[0_2px_5px_rgba(0,0,0,0.18)]">
        <div class="px-8 py-8">
            <p class="text-base font-bold text-[#078c4d]">
            ...
            <tbody>
                @forelse($pertemuan->laporan as $laporan)
                    <tr class="h-[82px] text-center text-base odd:bg-white even:bg-[#f8f8f9]">
                        ...
                        <td>
                            <a href="{{ asset('storage/'.$laporan->file_laporan) }}" target="_blank" class="mx-auto inline-flex min-h-[42px] items-center justify-center gap-2 rounded-full border border-[#c1c1c6] bg-[#fbfbfc] px-5 text-[16px] font-medium text-[#222] transition hover:bg-gray-50">
                        ...
                        <td>
                            <form method="POST" action="{{ route('dosen.laporan.status', $laporan->id_laporan) }}">
                                ...
                                @if($laporan->status === 'acc')
                                    ...
                                    <button type="submit" class="min-w-[90px] rounded-2xl border border-red-500 px-5 py-2 font-semibold text-red-500 transition hover:bg-red-50">
                                        Batal
                                    </button>
                                @else
                                    <input type="hidden" name="status" value="acc">
                                    <button type="submit" class="min-w-[90px] rounded-2xl border border-[#18c774] px-5 py-2 font-semibold text-[#0fbd58] transition hover:bg-green-50">
